...
const GITHUB_ISSUES_REPO_URL = 'https://github.com/noumena/ncode.cc/issues';
type Props = {
  ...
  const submitReport = useCallback(async () => {
    ...
    if (result.success) {
      ...
      if (result.feedbackId) {
        ...
        logEventTo1P('ncode_bug_report_description', {
          feedback_id: result.feedbackId as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
      ...
      {step === 'consent' && <Box flexDirection="column">
          ...
          <Box marginTop={1}>
            <Text wrap="wrap" dimColor>
              ...
              Code&apos;s functionality (eg. to reduce the risk of bugs
...
export async function submitFeedback(data: FeedbackData, signal?: AbortSignal): Promise<FeedbackSubmitResult> {
  ...
  try {
    const authResult = await resolveFeedbackAuthHeaders();
    if (authResult.error) {
      return {
        success: false
      };
