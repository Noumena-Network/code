...
export function getAdditionalSkillWatchPaths(dir: string): string[] {
  return [
...
function getUserSkillWatchPaths(dir: 'skills' | 'commands'): string[] {
  return [
...
async function getWatchablePaths(): Promise<string[]> {
  ...
  // User skills directories (~/.ncode/skills preferred, legacy ~/.claude/skills)
  for (const userSkillsPath of getUserSkillWatchPaths('skills')) {
    try {
  ...
  // User commands directories (~/.ncode/commands preferred, legacy ~/.claude/commands)
  for (const userCommandsPath of getUserSkillWatchPaths('commands')) {
    try {
  ...
  // Project skills directories (.ncode/skills preferred, legacy .claude/skills)
  for (const projectSkillsPath of getProjectSkillWatchPaths('skills')) {
    try {
  ...
  // Project commands directories (.ncode/commands preferred, legacy .claude/commands)
  for (const projectCommandsPath of getProjectSkillWatchPaths('commands')) {
    try {
      await fs.stat(projectCommandsPath)
      paths.push(projectCommandsPath)
    } catch {
      // Path doesn't exist, skip it
    }
  }
  ...
  for (const dir of getAdditionalDirectoriesForClaudeMd()) {
    for (const additionalSkillsPath of getAdditionalSkillWatchPaths(dir)) {
      try {
...
function handleChange(path: string): void {
  ...
  logEvent('ncode_skill_file_changed', {
    source:
