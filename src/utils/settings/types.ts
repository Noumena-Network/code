...
const BuiltInStatusLineConfigSchema = z.object({
  type: z.literal('builtin'),
  pathDisplay: z
    .enum(['full', 'short', 'hidden'])
    .optional()
    .describe(
      'Controls how the built-in status line displays the current working directory',
    ),
})

const CommandStatusLineConfigSchema = z.object({
  ...
  z
    .object({
      ...
      defaultMode: z
        ...
        .optional()
        .describe('Default permission mode when Code needs access'),
 ...
 *
 * This schema defines the structure of user settings files
  ...
  z
    .object({
      $schema: z
        ...
        .optional()
        .describe('JSON Schema reference for Code settings'),
      apiKeyHelper: z
        .string()
        .optional()
        ...
        ? {
            xaaIdp: z
              .object({
                ...
                clientId: z
                  .string()
                  .describe("Code's client_id registered at the IdP"),
                callbackPort: z
                  .number()
                  .int()
                  .positive()
      ...
      env: EnvironmentVariablesSchema()
        .optional()
        .describe('Environment variables to set for Code sessions'),
      ...
      attribution: z
        ...
        .describe(
          'Customize attribution text for commits and PRs. ' +
            'Each field defaults to the standard Code attribution if not set.',
        ),
      includeCoAuthoredBy: z
        ...
        .describe(
          'Deprecated: Use attribution instead. ' +
            "Whether to include Code's co-authored by attribution in commits and PRs (defaults to true)",
        ),
      includeGitInstructions: z
        .boolean()
        .optional()
        .describe(
          "Include built-in commit and PR workflow instructions in Code's system prompt (default: true)",
        ),
      ...
      model: z
        .string()
        .optional()
        .describe('Override the default model used by Code'),
      ...
      strictPluginOnlyCustomization: z
        ...
        .describe(
          'When set in managed settings, blocks non-plugin customization sources for the listed surfaces. ' +
            ...
            'Blocked: ~/.ncode/{surface}/, .ncode/{surface}/ (project), settings.json hooks, .mcp.json. Legacy ~/.claude/{surface}/ and .claude/{surface}/ still supported. ' +
      ...
      statusLine: z
        .union([CommandStatusLineConfigSchema, BuiltInStatusLineConfigSchema])
        .optional()
        .describe(
          'Status line configuration for either the built-in footer or a custom command status line',
        ),
      ...
      extraKnownMarketplaces: z
        ...
        .optional()
        .describe(
          'Additional marketplaces to make available for this repository. Typically used in repository .ncode/settings.json (legacy .claude/settings.json) to ensure team members have required plugin sources.',
        ),
      ...
      // Force a specific login method: 'claudeai' for Pro/Max, 'console' for Console billing
      forceLoginMethod: z
      ...
      language: z
        ...
        .optional()
        .describe(
          'Preferred language for NCode responses and voice dictation (e.g., "japanese", "spanish")',
        ),
      ...
      plansDirectory: z
        ...
        .describe(
          'Custom directory for plan files, relative to project root. ' +
            'If not set, defaults to ~/.ncode/plans/ (legacy ~/.claude/plans/ still supported).',
        ),
      ...(isInternalBuild()
        ? {
      ...
      ...(feature('KAIROS')
        ? {
            assistant: z
              ...
              .optional()
              .describe(
                'Start NCode in assistant mode (custom system prompt, brief view, scheduled check-in skills)',
              ),
            assistantName: z
              .string()
              .optional()
      ...
      autoMemoryEnabled: z
        ...
        .optional()
        .describe(
          'Enable auto-memory for this project. When false, NCode will not read from or write to the auto-memory directory.',
        ),
      autoMemoryDirectory: z
        .string()
        .optional()
        .describe(
          'Custom directory path for auto-memory storage. Supports ~/ prefix for home directory expansion. Ignored if set in projectSettings (checked-in .ncode/settings.json; legacy .claude/settings.json) for security. When unset, defaults to ~/.ncode/projects/<sanitized-cwd>/memory/ (legacy ~/.claude/projects/<sanitized-cwd>/memory/ still supported).',
        ),
      autoDreamEnabled: z
        .boolean()
        .optional()
            ...
            startDirectory: z
              ...
              .describe(
                'Default working directory on the remote host. ' +
                  ...
                  'Can be overridden by the [dir] positional argument in `ncode ssh <config> [dir]>`.',
              ),
          }),
        )
        .optional()
      ...
      claudeMdExcludes: z
        ...
        .optional()
        .describe(
          'Glob patterns or absolute paths of NCODE.md / legacy CLAUDE.md files to exclude from loading. ' +
            'Patterns are matched against absolute file paths using picomatch. ' +
            'Only applies to User, Project, and Local memory types (Managed/policy files cannot be excluded). ' +
            'Examples: "/home/user/monorepo/NCODE.md", "**/code/CLAUDE.md", "**/some-dir/.ncode/rules/**" (legacy .claude/rules also supported)',
        ),
