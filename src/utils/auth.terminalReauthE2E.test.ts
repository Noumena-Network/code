...
beforeEach(async () => {
  tempConfigDir = await mkdtemp(join(tmpdir(), 'ncode-auth-terminal-reauth-'))
  restoreEnv()
...
describe('terminal managed re-auth end-to-end', () => {
  it('repairs expired managed auth after refresh invalidation by completing a real browser login flow', async () => {
    ...
    await withMockOauthEnvironment(server, async () => {
      ...
      expect(cachedTokens).toMatchObject({
        accessToken: 'access-token',
        refreshToken: 'refresh-token',
      })
    })
