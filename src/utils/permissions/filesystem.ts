...
import {
  ...
  GLOBAL_NCODE_FOLDER_PERMISSION_PATTERN,
  NCODE_FOLDER_PERMISSION_PATTERN,
 ...
 * require granting session access to all of the managed config directory
 * (settings.json, hooks/, etc.).
 */
export function getNcodeSkillScope(
  filePath: string,
  ...
  const bases = [
    {
      dir: expandPath(join(getOriginalCwd(), '.ncode', 'skills')),
      prefix: '/.ncode/skills/',
    },
    {
...
export function isNcodeSettingsPath(filePath: string): boolean {
  // SECURITY: Normalize path structure first to prevent bypass via redundant ./
  // sequences like `./.ncode/./settings.json` or `./.claude/./settings.json`
  // which would evade the endsWith() check
  ...
  if (
    normalizedPath.endsWith(`${sep}.ncode${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.ncode${sep}settings.local.json`) ||
    normalizedPath.endsWith(`${sep}.claude${sep}settings.json`) ||
    normalizedPath.endsWith(`${sep}.claude${sep}settings.local.json`)
  ) {
    // Include managed project settings paths even for other projects
    return true
  }
...
// Always ask when NCode tries to edit its own config files
function isNcodeConfigFilePath(filePath: string): boolean {
  if (isNcodeSettingsPath(filePath)) {
    return true
  }
 ...
 * Path format: ~/.ncode/projects/{sanitized-cwd}/...
 */
 ...
 * The scratchpad is a per-session directory for NCode to write temporary files.
 * Controlled by the ncode_scratch Statsig gate.
 */
export function isScratchpadEnabled(): boolean {
  return checkStatsigFeatureGate_CACHED_MAY_BE_STALE('ncode_scratch')
}

/**
 * Returns the user-specific NCode temp directory name.
 * On Unix: 'ncode-{uid}' to prevent multi-user permission conflicts
 * On Windows: 'ncode' (tmpdir() is already per-user)
 */
export function getNcodeTempDirName(): string {
  if (getPlatform() === 'windows') {
    return 'ncode'
  }
...
/**
 * Returns the NCode temp directory path with symlinks resolved.
 * Uses TMPDIR env var if set, otherwise:
 * - On Unix: /tmp/ncode-{uid}/ (resolved to /private/tmp/ncode-{uid}/ on macOS)
 * - On Windows: {tmpdir}/ncode/ (e.g., C:\Users\{user}\AppData\Local\Temp\ncode\)
 * This is a per-user temporary directory used by NCode for all temp files.
 *
 ...
 * resolution, paths like /tmp/ncode-{uid}/... wouldn't match /private/tmp/ncode-{uid}/...
 */
...
export const getNcodeTempDir = memoize(function getNcodeTempDir(): string {
  const baseTmpDir =
  ...
  return join(resolvedBaseTmpDir, getNcodeTempDirName()) + sep
})
...
export const getBundledSkillsRoot = memoize(
  function getBundledSkillsRoot(): string {
    ...
    return join(getNcodeTempDir(), 'bundled-skills', MACRO.VERSION, nonce)
  },
)

/**
 * Returns the project temp directory path with trailing separator.
 * Path format: /tmp/ncode-{uid}/{sanitized-cwd}/
 */
export function getProjectTempDir(): string {
  return join(getNcodeTempDir(), sanitizePath(getOriginalCwd())) + sep
}

/**
 * Returns the scratchpad directory path for the current session.
 * Path format: /tmp/ncode-{uid}/{sanitized-cwd}/{sessionId}/scratchpad/
 */
...
function isScratchpadPath(absolutePath: string): boolean {
  ...
  //   echo "malicious" > /tmp/ncode-0/proj/session/scratchpad/../../../etc/passwd
...
function isDangerousFilePathToAutoEdit(path: string): boolean {
  ...
  for (let i = 0; i < pathSegments.length; i++) {
    ...
    for (const dir of DANGEROUS_DIRECTORIES) {
      ...
      // Special case: .claude/worktrees/ is a structural path (where NCode stores
 ...
 * - NCode config files (.ncode/settings.json, .ncode/commands/, .ncode/agents/)
 * - MCP CLI state files (managed internally by NCode)
...
export function checkPathSafetyForAutoEdit(
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        safe: false,
        message: `NCode requested permissions to write to ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        classifierApprovable: false,
      }
    }
  }

  // Check for NCode config files on all paths
  for (const pathToCheck of pathsToCheck) {
    if (isNcodeConfigFilePath(pathToCheck)) {
      return {
        safe: false,
        message: `NCode requested permissions to write to ${path}, but you haven't granted it yet.`,
  ...
  for (const pathToCheck of pathsToCheck) {
    if (isDangerousFilePathToAutoEdit(pathToCheck)) {
      return {
        safe: false,
        message: `NCode requested permissions to edit ${path} which is a sensitive file.`,
        classifierApprovable: true,
      }
    }
  }
...
export function checkReadPermissionForTool(
  ...
  if (typeof tool.getPath !== 'function') {
    return {
      behavior: 'ask',
      message: `NCode requested permissions to use ${tool.name}, but you haven't granted it yet.`,
    }
  }
  ...
  for (const pathToCheck of pathsToCheck) {
    if (pathToCheck.startsWith('\\\\') || pathToCheck.startsWith('//')) {
      return {
        behavior: 'ask',
        message: `NCode requested permissions to read from ${path}, which appears to be a UNC path that could access network resources.`,
        decisionReason: {
          type: 'other',
  ...
  for (const pathToCheck of pathsToCheck) {
    if (hasSuspiciousWindowsPathPattern(pathToCheck)) {
      return {
        behavior: 'ask',
        message: `NCode requested permissions to read from ${path}, which contains a suspicious Windows path pattern that requires manual approval.`,
        decisionReason: {
          type: 'other',
          reason:
  ...
  for (const pathToCheck of pathsToCheck) {
    ...
    if (askRule) {
      return {
        behavior: 'ask',
        message: `NCode requested permissions to read from ${path}, but you haven't granted it yet.`,
        decisionReason: {
          type: 'rule',
          rule: askRule,
        },
...
export function checkWritePermissionForTool<Input extends AnyObject>(
  ...
  // 1.6. Check for managed config folder allow rules BEFORE safety checks.
  // This allows session-level permissions to bypass the safety blocks for the
  // managed config directory (.ncode or legacy .claude).
  // We only allow this for session-level rules to prevent users from accidentally
  // permanently granting broad access to their managed config folder.
  //
  ...
  // "allow NCode to edit its own settings for this session" option actually works.
  ...
  if (claudeFolderAllowRule) {
    // Check if this rule is scoped under the managed config folder (project or
    // global). Accepts both canonical .ncode and legacy .claude broad patterns
    // as well as narrowed skill rules, so users can grant
    ...
    // '/.ncode/../**' from leaking this bypass outside the config folder.
    const ruleContent = claudeFolderAllowRule.ruleValue.ruleContent
    if (
      ruleContent &&
      (ruleContent.startsWith(NCODE_FOLDER_PERMISSION_PATTERN.slice(0, -2)) ||
        ruleContent.startsWith(
          GLOBAL_NCODE_FOLDER_PERMISSION_PATTERN.slice(0, -2),
        ) ||
  ...
  // 1.7. Check comprehensive safety validations (Windows patterns, NCode config, dangerous files)
  ...
  if (!safetyCheck.safe) {
    // SDK suggestion: if under the managed skills directory, emit the narrowed
    // session-scoped addRules that step 1.6 will honor on the next call.
    // Everything else (.ncode/settings.json, legacy .claude/settings.json,
    ...
    const skillScope = getNcodeSkillScope(path)
...
export function checkEditableInternalPath(
  ...
  // The desktop's preview_start MCP tool instructs NCode to create/update
