...
// NCode configuration directory names
export const NCODE_CONFIG_DIRECTORIES = [
  'commands',
  'agents',
  'output-styles',
  'skills',
...
export type NcodeConfigDirectory = (typeof NCODE_CONFIG_DIRECTORIES)[number]
...
function pushIfExistingDir(dirs: string[], dirPath: string): void {
  try {
    statSync(dirPath)
...
export function getExistingProjectOrManagedDirs(
  root: string,
...
function getExistingUserDirs(subdir: NcodeConfigDirectory): string[] {
  const dirs: string[] = []
 ...
 * stopping there makes the parent project's instruction directory unreachable (#31905).
 *
 ...
 *
 * Worktrees (under `.ncode/worktrees/` by default, legacy `.claude/worktrees/`)
 * stay on the old behavior: their `.git`
 ...
 * collecting both legacy .claude and canonical .ncode directories along the way.
 *
 ...
 * @returns Array of directory paths containing .claude/.ncode subdirs, from most specific (cwd) to least specific
 */
...
export const loadMarkdownFilesForSubdir = memoize(
  async function (
    ...
    const userDirs = getExistingUserDirs(subdir)
    const managedDirs = getExistingProjectOrManagedDirs(
      getManagedFilePath(),
      subdir,
    )
    ...
    if (gitRoot && canonicalRoot && canonicalRoot !== gitRoot) {
      const worktreeHasSubdir = getExistingProjectOrManagedDirs(
        gitRoot,
        subdir,
      ).some(worktreeSubdir =>
        projectDirs.some(
          dir =>
            ...
            normalizePathForComparison(worktreeSubdir),
        ),
      )
      if (!worktreeHasSubdir) {
        for (const mainConfigDir of getExistingProjectOrManagedDirs(
          canonicalRoot,
          subdir,
        )) {
          if (!projectDirs.includes(mainConfigDir)) {
            projectDirs.push(mainConfigDir)
          }
        }
      }
    }

    const [managedFilesNested, userFilesNested, projectFilesNested] = await Promise.all([
    ...
    const managedFiles = managedFilesNested.flat()
  ...
  (subdir: NcodeConfigDirectory, cwd: string) => `${subdir}:${cwd}`,
)
