...
/**
 * --bare / NCODE_SIMPLE — skip hooks, LSP, plugin sync, skill dir-walk,
 * attribution, background prefetches, and ALL keychain/credential reads.
 * Legacy Anthropic API-key compatibility is strictly ANTHROPIC_API_KEY env
 * or apiKeyHelper from --settings.
 * Explicit CLI flags (--plugin-dir, --add-dir, --mcp-config) still honored.
 * ~30 gates across the codebase.
 *
 * Checks argv directly (in addition to the env var) because several gates
 * run before main.tsx's action handler sets NCODE_SIMPLE=1 from --bare
 * — notably startKeychainPrefetch() at main.tsx top-level.
 */
export function isBareMode(): boolean {
  return (
    isEnvTruthy(process.env.NCODE_SIMPLE) ||
 ...
 * @returns true if NCODE_BASH_MAINTAIN_PROJECT_WORKING_DIR or its legacy
 * compatibility alias is set to a truthy value
 */
export function shouldMaintainProjectWorkingDir(): boolean {
  return isEnvTruthy(
    process.env.NCODE_BASH_MAINTAIN_PROJECT_WORKING_DIR ??
...
/**
 * Conservative check for whether NCode is running inside a protected
