/**
 * Adapter layer that wraps @anthropic-ai/sandbox-runtime with NCode-specific integrations.
 * This file provides the bridge between the external sandbox-runtime package and NCode's
...
import { getNcodeTempDir } from '../permissions/filesystem.js'
...
/**
 * Resolve NCode-specific path patterns for sandbox-runtime.
 *
 * NCode uses special path prefixes in permission rules:
...
export function getProtectedSettingsPathsForCwd(cwd: string): string[] {
  return [
    resolve(cwd, '.ncode', 'settings.json'),
    resolve(cwd, '.ncode', 'settings.local.json'),
    resolve(cwd, '.claude', 'settings.json'),
    resolve(cwd, '.claude', 'settings.local.json'),
  ]
}

export function getProtectedSkillsPathsForCwd(cwd: string): string[] {
  return [
...
/**
 * Convert NCode settings format to SandboxRuntimeConfig format
...
export function convertToSandboxRuntimeConfig(
  ...
  // Always include current directory and NCode temp directory as writable
  // The temp directory is needed for Shell.ts cwd tracking files
  const allowWrite: string[] = ['.', getNcodeTempDir()]
  const denyWrite: string[] = []
  const denyRead: string[] = []
  const allowRead: string[] = []

  // Always deny writes to settings.json files to prevent sandbox escape
  // This blocks settings in the original working directory (where NCode started)
  ...
  denyWrite.push(...getProtectedSkillsPathsForCwd(originalCwd))
  if (cwd !== originalCwd) {
    denyWrite.push(...getProtectedSkillsPathsForCwd(cwd))
  }

  // SECURITY: Git's is_git_directory() treats cwd as a bare repo if it has
  // HEAD + objects/ + refs/. An attacker planting these (plus a config with
  // core.fsmonitor) escapes the sandbox when NCode's unsandboxed git runs.
  //
 ...
 * NCode's unsandboxed git calls can see them. See the SECURITY block above
 ...
 * This is an NCode-specific function that updates local settings.
 */
...
/**
 * NCode sandbox manager - wraps sandbox-runtime with NCode-specific features
 */
