...
function findMcpServerConnection(
  ...
  // is the original name (e.g., "managed Slack"). Normalize both for comparison.
...
export async function* runToolUse(
  ...
  try {
    if (toolUseContext.abortController.signal.aborted) {
      logEvent('ncode_tool_use_cancelled', {
...
function streamedCheckPermissionsAndCallTool(
  ...
  checkPermissionsAndCallTool(
    ...
    mcpServerBaseUrl,
    progress => {
      logEvent('ncode_tool_use_progress', {
        messageID:
...
async function checkPermissionsAndCallTool(
  ...
  if (!parsedInput.success) {
    ...
    if (schemaHint) {
      logEvent('ncode_deferred_tool_schema_not_sent', {
        toolName: sanitizeToolNameForAnalytics(tool.name),
        isMcp: tool.isMcp ?? false,
      })
  ...
  if (permissionDecision.behavior !== 'allow') {
    ...
    logEvent('ncode_tool_use_can_use_tool_rejected', {
      messageID:
        messageId as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
      toolName: sanitizeToolNameForAnalytics(tool.name),
  ...
  logEvent('ncode_tool_use_can_use_tool_allowed', {
    messageID:
      messageId as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
    toolName: sanitizeToolNameForAnalytics(tool.name),
  ...
  try {
    ...
    logEvent('ncode_tool_use_success', {
      messageID:
        messageId as AnalyticsMetadata_I_VERIFIED_THIS_IS_NOT_CODE_OR_FILEPATHS,
      toolName: sanitizeToolNameForAnalytics(tool.name),
      isMcp: tool.isMcp ?? false,
