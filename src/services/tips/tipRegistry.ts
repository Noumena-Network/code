...
import { isCurrentFirstPartyApiCustomer } from '../../utils/firstPartyApiCustomerSession.js'
...
const externalTips: Tip[] = [
  {
    id: 'new-user-warmup',
    content: async () =>
      `Start with small features or bug fixes, tell Code to propose a plan, and verify its suggested edits`,
    cooldownSessions: 3,
    async isRelevant() {
  ...
  },
  {
    id: 'git-worktrees',
    content: async () =>
      'Use git worktrees to run multiple Code sessions in parallel.',
    cooldownSessions: 10,
    isRelevant: async () => {
      try {
        const config = getGlobalConfig()
  ...
  },
  {
    ...
    content: async () =>
      'Running multiple Code sessions? Use /color and /rename to tell them apart at a glance.',
    cooldownSessions: 10,
    isRelevant: async () => {
  ...
  {
    id: 'memory-command',
    content: async () => 'Use /memory to view and manage Code memory',
    cooldownSessions: 15,
    async isRelevant() {
      const config = getGlobalConfig()
  ...
  },
  {
    ...
    cooldownSessions: 25,
    isRelevant: async () =>
      getSettings_DEPRECATED().statusLine?.type !== 'command',
  },
  {
    id: 'prompt-queue',
    content: async () =>
      'Hit Enter to queue up additional messages while Code is working.',
    cooldownSessions: 5,
    async isRelevant() {
      const config = getGlobalConfig()
  ...
  },
  {
    ...
    content: async () =>
      'Send messages to Code while it works to steer Code in real-time',
    cooldownSessions: 20,
    isRelevant: async () => true,
  },
  {
    id: 'todo-list',
    content: async () =>
      'Ask Code to create a todo list when working on complex tasks to track progress and remain on track',
    cooldownSessions: 20,
    isRelevant: async () => true,
  },
  {
  ...
  },
  {
    ...
    content: async () => 'Connect Code to your IDE · /ide',
    cooldownSessions: 4,
    async isRelevant() {
  ...
  },
  {
    ...
    content: async () => 'Run /install-slack-app to use NCode in Slack',
    cooldownSessions: 10,
  ...
  },
  {
    id: 'paste-images-mac',
    content: async () =>
      'Paste images into Code using control+v (not cmd+v!)',
    cooldownSessions: 10,
  ...
  },
  {
    id: 'continue',
    content: async () =>
      'Run ncode --continue or ncode --resume to resume a conversation',
    cooldownSessions: 10,
    isRelevant: async () => true,
  },
  {
  ...
  },
  {
    id: 'desktop-app',
    content: async () =>
      'Run Code locally or remotely using the NCode desktop app: console.noumena.com/desktop',
    cooldownSessions: 15,
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      return `Continue your session in Code with ${blue('/desktop')}`
    },
    cooldownSessions: 15,
    isRelevant: async () => {
  ...
  },
  {
    id: 'mobile-app',
    content: async () =>
      '/mobile to use Code from the Noumena app on your phone',
    cooldownSessions: 15,
    isRelevant: async () => true,
  },
  {
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      return variant === 'copy_b'
        ? `Use ${cmd} for better one-shot answers. Code thinks it through first.`
        : `Working on something tricky? ${cmd} gives better first answers`
    },
    cooldownSessions: 3,
    isRelevant: async () => {
      if (!isCurrentFirstPartyApiCustomer()) return false
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      const variant = getFeatureValue_CACHED_MAY_BE_STALE<
        'off' | 'copy_a' | 'copy_b'
      >('ncode_tern_alloy', 'off')
      return variant === 'copy_b'
        ? `For big tasks, tell Code to ${blue('use subagents')}. They work in parallel and keep your main thread clean.`
        : `Say ${blue('"fan out subagents"')} and Code sends a team. Each one digs deep so nothing gets missed.`
    },
    cooldownSessions: 3,
    isRelevant: async () => {
      if (!isCurrentFirstPartyApiCustomer()) return false
  ...
  },
  {
    ...
    content: async ctx => {
      ...
      return reward
        ? `Share Code and earn ${claude(formatCreditAmount(reward))} of extra usage · ${claude('/passes')}`
    ...
    ? [
        {
          ...
          content: async () =>
            '[NOUMENA-ONLY] Use "IMPORTANT:" prefix for must-follow NCODE.md rules',
          cooldownSessions: 30,
          isRelevant: async () => true,
        },
        {
          id: 'skillify',
          content: async () =>
            '[NOUMENA-ONLY] Use /skillify at the end of a workflow to turn it into a reusable skill',
          cooldownSessions: 15,
          isRelevant: async () => true,
        },
      ]
