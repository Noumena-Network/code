...
import {
  ...
  getExistingProjectOrManagedDirs,
  ...
  async (cwd: string): Promise<Command[]> => {
    ...
    if (isBareMode()) {
      ...
      const additionalSkillsNested = await Promise.all(
        additionalDirs.flatMap(dir =>
          getAdditionalManagedSkillDirs(dir).map(skillDir =>
    ...
    const [
      ...
      isEnvTruthy(process.env.CLAUDE_CODE_DISABLE_POLICY_SKILLS)
        ? Promise.resolve([])
        : Promise.all(
            managedSkillsDirs.map(managedSkillsDir =>
...
export async function discoverSkillDirsForPaths(
  ...
  for (const filePath of filePaths) {
    ...
    while (currentDir.startsWith(resolvedCwd + pathSep)) {
      for (const skillDir of getAdditionalManagedSkillDirs(currentDir)) {
        ...
        if (!dynamicSkillDirs.has(skillDir)) {
          ...
          try {
            ...
            // is gitignored — blocks e.g. node_modules/pkg/.ncode/skills and
            // legacy node_modules/pkg/.claude/skills from loading silently.
...
export async function addSkillDirectories(dirs: string[]): Promise<void> {
  ...
  if (newSkillCount > 0) {
    ...
    if (addedSkills.length > 0) {
      logEvent('ncode_dynamic_skills_changed', {
        source:
